...
import { CheckCircle, XCircle, AlertTriangle, Clock, Zap, Wallet, Shield, TrendingUp, Plus, Activity, ArrowUpRight, ArrowDownLeft, Smartphone, Wifi, LayoutDashboard, CreditCard, Users, Home } from 'lucide-react'
    ...
    <div style={{
      ...
      {[
        ...
        <Link key={path} to={path} style={{
          ...
          padding:'4px 20px', color: active(path) ? 'var(--squad-orange)' : 'rgba(255,255,255,0.3)',
          transition:'color 0.15s',
        }}>
          <Icon size={22} strokeWidth={active(path) ? 2.5 : 1.5} />
          <span style={{ fontSize:10, fontWeight: active(path) ? 700 : 400, letterSpacing:'0.3px' }}>{label}</span>
        </Link>
      ))}
    </div>
  )
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // if (sessionsCompleted === 0 && !enrolled) {
    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#07050A' }}>
      <div style={{ textAlign:'center' }}>
        <div style={{ width:52, height:52, borderRadius:14, margin:'0 auto 14px',
          border:'1px solid rgba(244,82,30,0.2)', background:'rgba(244,82,30,0.05)',
          display:'flex', alignItems:'center', justifyContent:'center', animation:'rotate3d 3s ease-in-out infinite' }}>
  ...
  return (
    <div style={{ background:'#07050A', minHeight:'100vh' }}>
      <div className="page" style={{ padding: isMobile ? '72px 16px 100px' : '80px 24px 48px', maxWidth:1100, margin:'0 auto' }}>
          ...
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
              ...
              <span style={{ fontSize:11, color:'var(--squad-orange)', fontFamily:'var(--mono)', letterSpacing:'0.5px' }}>
                {stats?.is_enrolled ? 'PROTECTED' : 'ENROLLING'}
              </span>
            </div>
            <h1 style={{ fontSize: isMobile ? 24 : 30, fontWeight:700, letterSpacing:'-0.8px', marginBottom:2,
              fontFamily:'"Georgia",serif' }}>
              Good {new Date().getHours()<12?'morning':new Date().getHours()<17?'afternoon':'evening'},{' '}
              <span style={{ color:'var(--squad-orange)' }}>{user?.full_name?.split(' ')[0]}</span>
            </h1>
            <p style={{ color:'rgba(255,255,255,0.4)', fontSize:13 }}>
              Your behavioral security · powered by Squad
            </p>
          </div>
          {!isMobile && (
          ...
          <div className="anim-fade-up d1" style={{
            ...
            <div style={{ flex:1 }}>
              <div style={{ fontWeight:600, fontSize:13, marginBottom:4 }}>
                Building profile — {5 - stats.enrollment_sessions} session{5-stats.enrollment_sessions!==1?'s':''} remaining
              </div>
              <div style={{ background:'rgba(255,255,255,0.06)', borderRadius:2, height:3, overflow:'hidden', maxWidth:180 }}>
                <div style={{ height:'100%', borderRadius:2, background:'var(--gold)', width:`${(stats.enrollment_sessions/5)*100}%`, transition:'width 0.5s' }} />
              </div>
            </div>
            <Link to="/onboarding" className="btn btn-ghost" style={{ fontSize:12, padding:'6px 12px' }}>
              Continue →
            </Link>
          </div>
        )}
        ...
        <div className="card anim-fade-up d1 card-3d" style={{
          marginBottom:14, padding: isMobile ? '20px' : '24px 28px',
          background:'#0A0608', border:'1px solid rgba(244,82,30,0.2)',
          boxShadow:'0 8px 40px rgba(0,0,0,0.5)',
        }}>
          {/* Shimmer — very subtle */}
          <div style={{ position:'absolute', inset:0, background:'linear-gradient(105deg, transparent 30%, rgba(244,82,30,0.025) 50%, transparent 70%)',
            backgroundSize:'200% 100%', animation:'shimmer 5s linear infinite', pointerEvents:'none' }} />
          ...
          <div style={{ position:'relative', display:'flex', alignItems:'flex-start', justifyContent:'space-between', flexWrap:'wrap', gap:16 }}>
            <div style={{ flex:1, minWidth:180 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16 }}>
                <div style={{ width:36, height:36, borderRadius:10, background:'var(--squad-orange)',
              ...
              <div style={{ marginBottom:16 }}>
                ...
                <div style={{ fontFamily:'"Georgia",serif', fontSize: isMobile ? 36 : 44, fontWeight:700, letterSpacing:'-1px', lineHeight:1 }}>
                  <span style={{ fontSize: isMobile ? 18 : 22, color:'rgba(255,255,255,0.4)', marginRight:4 }}>₦</span>
                  <span>0.00</span>
                </div>
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.25)', marginTop:4 }}>Fund wallet to start transacting</div>
              </div>
              <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                ...
                <button className="btn btn-outline" style={{ fontSize:13, padding:'9px 16px' }}>
            ...
            {!isMobile && (
              <div style={{ textAlign:'center' }}>
                <ScoreRing score={avg} size={90} showLabel={false} />
                <div style={{ fontSize:11, color:'rgba(255,255,255,0.3)', marginTop:6 }}>Avg Score</div>
                <div style={{ fontSize:13, fontWeight:700, color:healthColor, marginTop:2 }}>{health}</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="anim-fade-up d2" style={{ display:'grid', gridTemplateColumns:`repeat(${isMobile?4:6}, 1fr)`, gap:10, marginBottom:14 }}>
          {[
            ...
            ...(!isMobile ? [
              { label:'Transfer', icon:'🔄', action:() => {} },
              { label:'More', icon:'⋯', action:() => {} },
            ] : []),
          ].map(({ label, icon, action }) => (
            <button key={label} onClick={action} style={{
              background:'#0E0A0C', border:'1px solid rgba(255,255,255,0.06)',
        ...
        <div className="anim-fade-up d2" style={{
          ...
          {[
            ...
            { label:'Profile Vectors',value: profile?.total_vectors_stored||0, color:'#B37FEB', icon:'🧬' },
            { label:'Status',         value: stats?.is_enrolled?'Active':`${stats?.enrollment_sessions||0}/5`, color: stats?.is_enrolled?'var(--green)':'var(--gold)', icon:'✓' },
        ...
        <div className="anim-fade-up d3" style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:10, marginBottom:14 }}>
          ...
          <div className="card" style={{ padding:'18px 20px' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6, marginBottom:12 }}>
              {['₦50','₦100','₦200','₦500'].map(amt => (
                <button key={amt} style={{ padding:'8px 4px', borderRadius:7,
                  background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.06)',
                  color:'rgba(255,255,255,0.6)', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'var(--font)' }}>
                  {amt}
                </button>
              ))}
            </div>
          ...
          <div className="card" style={{ padding:'18px 20px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16 }}>
              <div style={{ width:34, height:34, borderRadius:9, background:'rgba(59,130,246,0.1)',
                display:'flex', alignItems:'center', justifyContent:'center' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6, marginBottom:12 }}>
              {['1GB','2GB','5GB','10GB'].map(plan => (
                <button key={plan} style={{ padding:'8px 4px', borderRadius:7,
                  background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.06)',
                  color:'rgba(255,255,255,0.6)', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'var(--font)' }}>
                  {plan}
                </button>
        ...
        <div className="anim-fade-up d3" style={{
          display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 360px', gap:12,
        }}>
          {/* Score chart */}
            ...
            <ResponsiveContainer width="100%" height={isMobile ? 140 : 180}>
              <AreaChart data={MOCK_CHART} margin={{ top:5, right:5, bottom:0, left:-20 }}>
                ...
                <Tooltip contentStyle={{ background:'#130D10', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8, fontSize:12 }}
                  labelStyle={{ color:'rgba(255,255,255,0.5)' }} itemStyle={{ color:'var(--squad-orange)' }} />
                <Area type="monotone" dataKey="score" stroke="var(--squad-orange)" strokeWidth={2}
                  fill="url(#scoreGrad)" dot={{ fill:'var(--squad-orange)', r:3, strokeWidth:0 }}
                  activeDot={{ r:5, fill:'var(--squad-orange)' }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
            ...
            <div style={{ overflowY:'auto', maxHeight: isMobile ? 200 : 240 }}>
              {sessions.length === 0 ? (
                <div style={{ padding:'28px 18px', textAlign:'center', color:'rgba(255,255,255,0.3)', fontSize:13 }}>
                  No sessions yet.<br />
                  <Link to="/pay" style={{ color:'var(--squad-orange)', fontWeight:600 }}>Make your first payment →</Link>
                </div>
              ) : sessions.map((s, i) => {
                ...
                return (
                  <div key={s.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'11px 18px',

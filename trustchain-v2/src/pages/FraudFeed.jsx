...
import { Shield, AlertTriangle, MapPin, Activity, CheckCircle } from 'lucide-react'
...
const LOCATIONS = ["Yaba, Lagos","Ikeja, Lagos","Victoria Island","Lekki Phase 1","Wuse 2, Abuja","Maitama, Abuja","Garki, Abuja","GRA, Port Harcourt","Kano Municipal","Bodija, Ibadan","GRA, Enugu","Ring Road, Benin","Uyo, Akwa Ibom","Aba, Abia","Asaba, Delta"]
const FRAUD_TYPES = ["SIM-swap keystroke mismatch","Abnormal touch pressure (3.2×)","Device orientation anomaly","Paste-based account takeover","High-speed form fill (bot-like)","Tap offset pattern mismatch","Gyroscope tilt deviation","Inter-key interval anomaly","Swipe velocity spike detected"]
const rnd = arr => arr[Math.floor(Math.random() * arr.length)]
const randAmt = () => Math.floor(Math.random() * 190000) + 10000
const randScore = () => Math.floor(Math.random() * 45) + 5
...
export default function FraudFeed() {
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      timerRef.current = setTimeout(() => {
        const amt = randAmt()
        const event = { id: Date.now().toString(), location: rnd(LOCATIONS), fraud_type: rnd(FRAUD_TYPES), amount_protected: amt, behavioral_score: randScore(), seconds_ago: 0, isNew: true }
        setLiveFeed(prev => [event, ...prev].slice(0, 14))
        setTotalBlocked(t => t + amt)
        setActiveCount(c => c + Math.floor(Math.random() * 3))
        setTimeout(() => setLiveFeed(prev => prev.map(e => e.id === event.id ? { ...e, isNew: false } : e)), 800)
        schedule()
  ...
  const chartData = [
    { t:'00:00', blocked:12 },{ t:'04:00', blocked:8 },{ t:'08:00', blocked:34 },
    { t:'12:00', blocked:67 },{ t:'16:00', blocked:89 },{ t:'20:00', blocked:112 },{ t:'Now', blocked:156 },
  ]
  const radarData = [
    { signal:'Keystroke', value:88 },{ signal:'Touch', value:74 },{ signal:'Swipe', value:91 },
    { signal:'Gyro', value:68 },{ signal:'Timing', value:85 },{ signal:'Tap', value:79 },
  ]
    ...
    <div style={{ padding: isMobile ? '72px 14px 100px' : '80px 24px 48px', background:'#07050A', minHeight:'100vh' }}>
      <div style={{ maxWidth:1100, margin:'0 auto' }}>
        ...
        <div className="anim-fade-up" style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:24, flexWrap:'wrap', gap:12 }}>
          <div>
            ...
            <p style={{ color:'rgba(255,255,255,0.4)', fontSize:13 }}>
              Live fraud intelligence across Nigeria · TrustChain + Squad
            </p>
          </div>
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            <span className="badge badge-red" style={{ animation:'pulse-glow 2s infinite' }}><Activity size={10} /> Live</span>
            <span className="badge badge-gray">{activeCount.toLocaleString()} active users</span>
            <span className="badge badge-orange">🇳🇬 Nigeria</span>
          </div>
        </div>
        ...
        <div className="anim-fade-up d1" style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4,1fr)', gap:10, marginBottom:16 }}>
          {[
            { label:'Amount Protected', value:`₦${(totalBlocked/1_000_000).toFixed(1)}M`, sub:'platform total', color:'var(--green)', icon:'🛡' },
            { label:'Fraud Blocked', value: data ? data.platform_stats.fraud_attempts_blocked.toLocaleString() : '—', sub:'payment attempts', color:'var(--red)', icon:'🚫' },
            { label:'Active Users', value: activeCount.toLocaleString(), sub:'protected now', color:'var(--squad-orange)', icon:'👥' },
            { label:'Prevention Rate', value: data ? `${data.platform_stats.fraud_prevention_rate}%` : '—', sub:'stopped', color:'#3B82F6', icon:'📊' },
        ...
        <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 360px', gap:12, marginBottom:12 }}>
          ...
          <div className="card" style={{ padding:0, overflow:'hidden' }}>
            <div style={{ padding:'14px 18px', borderBottom:'1px solid rgba(255,255,255,0.05)', display:'flex', alignItems:'center', gap:8 }}>
              ...
              <span className="badge badge-red" style={{ marginLeft:'auto' }}>Nigeria</span>
            </div>
            <div style={{ maxHeight: isMobile ? 340 : 440, overflowY:'auto' }}>
              {liveFeed.length === 0 ? (
                <div style={{ padding:32, textAlign:'center', color:'rgba(255,255,255,0.3)', fontSize:13 }}>
                  Waiting for events...
                </div>
              ) : liveFeed.map((event, i) => (
                <div key={event.id} style={{ display:'flex', alignItems:'flex-start', gap:12, padding:'12px 18px', borderBottom:'1px solid rgba(255,255,255,0.04)', background: event.isNew ? 'rgba(239,68,68,0.06)' : 'transparent', transition:'background 0.8s', animation: event.isNew ? 'fadeUp 0.3s ease' : 'none' }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:13, fontWeight:600, color:'var(--red)', marginBottom:2 }}>
                      {event.isNew && <span style={{ marginRight:6, fontSize:9, background:'var(--red)', color:'#fff', padding:'1px 6px', borderRadius:3, fontFamily:'var(--mono)' }}>NEW</span>}
                      BLOCKED — ₦{event.amount_protected?.toLocaleString()}
                    </div>
                    <div style={{ fontSize:12, color:'rgba(255,255,255,0.5)', marginBottom:3 }}>{event.fraud_type}</div>
                    <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                      <span style={{ display:'flex', alignItems:'center', gap:4, fontSize:11, color:'rgba(255,255,255,0.3)' }}><MapPin size={10} />{event.location}</span>
                      <span style={{ fontSize:11, color:'rgba(255,255,255,0.2)', fontFamily:'var(--mono)' }}>Score: {event.behavioral_score}/100</span>
                      <span style={{ fontSize:11, color:'rgba(255,255,255,0.2)' }}>{event.seconds_ago === 0 ? 'just now' : `${event.seconds_ago}s ago`}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="card" style={{ padding:'16px 18px' }}>
              ...
              <ResponsiveContainer width="100%" height={120}>
                <AreaChart data={chartData}>
                  ...
                  <Area type="monotone" dataKey="blocked" stroke="var(--red)" strokeWidth={2} fill="url(#rg)" dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </div>

            <div className="card" style={{ padding:'16px 18px' }}>
              <div style={{ fontSize:13, fontWeight:700, marginBottom:4 }}>Signal Detection Strength</div>
        ...
        {/* Nigeria badge */}
        <div className="card anim-fade-up" style={{ padding:'16px 20px', display:'flex', gap:14, alignItems:'center', flexWrap:'wrap' }}>
          <div style={{ fontSize:28 }}>🇳🇬</div>
          <div style={{ flex:1 }}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:4 }}>Nigeria Fraud Intelligence Network</div>
            <div style={{ fontSize:13, color:'rgba(255,255,255,0.45)', lineHeight:1.6 }}>
              Monitoring behavioral patterns across Lagos, Abuja, Port Harcourt, Kano and 32 other cities. Every blocked payment strengthens the model for all users. <strong style={{ color:'var(--squad-orange)' }}>Nigeria's first behavioral payment shield.</strong>
            </div>
          </div>
          <div style={{ textAlign:'right', flexShrink:0 }}>
            <div style={{ fontFamily:'"Georgia",serif', fontSize:24, fontWeight:700, color:'var(--green)' }}>₦{(totalBlocked/1_000_000).toFixed(1)}M</div>
